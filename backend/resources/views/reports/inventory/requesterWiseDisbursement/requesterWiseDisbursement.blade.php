...
<body>
    <table>
        <thead>
            ...
            <tr>
                <td colspan="8">
                    Requester Name: {{ $requesterInfo->name ?? '' }}
                    @if (!empty($requesterInfo->designation))
                    - [{{ $requesterInfo->designation->title ?? '' }}]
                    @endif
                    @if (!empty($requesterInfo->branch))
                    <br>
                    DMP Unit: {{ $requesterInfo->branch->name ?? '' }}
                    @endif
                </td>
            </tr>
            <tr>
