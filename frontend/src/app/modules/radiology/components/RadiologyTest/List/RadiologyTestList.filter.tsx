...
const RadiologyTestListFilter: FC<any> = (props) => {
  const {Search} = Input
        ...
        <Col md={6} xs={12}>
          <div className='card card-header p-0 pb-3' style={{minHeight: '0px'}}>
            <h3 className='card-title align-items-start flex-column'>
              <span className='card-label fw-bold fs-3 mb-1'>{t('Radiology Test Catalog')}</span>
            </h3>
          </div>
        </Col>
        <Col md={6} xs={12}>
          <CreateAction
            actionItem={RadiologyTestAction.COMMON_ACTION.CREATE}
...
export default React.memo(RadiologyTestListFilter)
