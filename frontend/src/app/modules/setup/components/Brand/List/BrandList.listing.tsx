...
const BrandListing: FC<any> = (props) => {
  const {
    loading,
    listData,
    selectedRowKeys,
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ViewAction
          entityId={record.id}
          actionItem={BrandAction.COMMON_ACTION.VIEW}
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ListItemAction
          entityId={record.id}
          actionList={BrandAction.LIST_ITEM_ACTION}
          handleCallbackFunc={handleCallbackFunc}
        />
      ),
    },
  ...
  return (
    <div className='px-6'>
      <AntTable
        ...
        rowSelection={false}
        rowSelectionPermission='auth:brand:multiSelect'
...
export default React.memo(BrandListing)
