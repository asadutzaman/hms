...
const AttributeListing: FC<any> = (props) => {
  const {
    loading,
    listData,
    selectedRowKeys,
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ViewAction
          entityId={record.id}
          actionItem={AttributeAction.COMMON_ACTION.VIEW}
  ...
  return (
    <div className='px-6'>
      <AntTable
        ...
        rowSelection={false}
        rowSelectionPermission='auth:attribute:multiSelect'
...
export default React.memo(AttributeListing)
