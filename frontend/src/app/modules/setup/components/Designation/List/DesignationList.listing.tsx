...
const DesignationListing: FC<any> = (props) => {
  const {t} = useLang()
  const {
    loading,
    listData,
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ViewAction
          entityId={record.id}
          actionItem={DesignationAction.COMMON_ACTION.VIEW}
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ListItemAction
          entityId={record.id}
          actionList={DesignationAction.LIST_ITEM_ACTION}
          handleCallbackFunc={handleCallbackFunc}
        />
      ),
    },
  ...
  return (
    <div className='px-6'>
      <AntTable
        ...
        rowSelection={false}
        rowSelectionPermission='auth:designation:multiSelect'
...
export default React.memo(DesignationListing)
