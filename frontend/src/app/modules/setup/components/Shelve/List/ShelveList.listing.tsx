...
const ShelveListing: FC<any> = (props) => {
  const {
    loading,
    listData,
    selectedRowKeys,
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ViewAction
          entityId={record.id}
          actionItem={ShelveAction.COMMON_ACTION.VIEW}
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ListItemAction
          entityId={record.id}
          actionList={ShelveAction.LIST_ITEM_ACTION}
          handleCallbackFunc={handleCallbackFunc}
        />
      ),
    },
...
export default React.memo(ShelveListing)
