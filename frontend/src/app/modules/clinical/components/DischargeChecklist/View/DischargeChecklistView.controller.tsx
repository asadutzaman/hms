...
import DischargeChecklistView from './DischargeChecklistView.view'

const initialState = {
  modalTitle: 'Discharge Checklist Info', itemData: {}, loading: false, fields: {},
  message: {network_error: 'A network error occurred. Please try again later.'},
}

const DischargeChecklistViewController: FC<any> = (props) => {
  const {BaseCrudViewService, modalTitle, itemData, setItemData, loading, entityId, reloadView, isShowView, handleCallbackFunc} =
    useCrudViewService(DischargeChecklistApi, initialState, props)
  useEffect(() => {
  ...
  return (
    <DrawerView loading={loading} reloadView={reloadView} isShowView={isShowView} modalTitle={modalTitle}
      itemData={itemData} component={DischargeChecklistView} handleCallbackFunc={handleCallbackFunc} />
  )
}
export default React.memo(DischargeChecklistViewController)
