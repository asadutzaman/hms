...
import DailyReviewView from './DailyReviewView.view'

const initialState = {
  modalTitle: 'Daily Review Info', itemData: {}, loading: false, fields: {},
  message: {network_error: 'A network error occurred. Please try again later.'},
}

const DailyReviewViewController: FC<any> = (props) => {
  const {BaseCrudViewService, modalTitle, itemData, setItemData, loading, entityId, reloadView, isShowView, handleCallbackFunc} =
    useCrudViewService(DailyReviewApi, initialState, props)
  useEffect(() => {
  ...
  return (
    <DrawerView loading={loading} reloadView={reloadView} isShowView={isShowView} modalTitle={modalTitle}
      itemData={itemData} component={DailyReviewView} handleCallbackFunc={handleCallbackFunc} />
  )
}
export default React.memo(DailyReviewViewController)
