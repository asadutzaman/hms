...
import CodeBlueEventView from './CodeBlueEventView.view'

const initialState = {
  modalTitle: 'Code Blue Event Info', itemData: {}, loading: false, fields: {},
  message: {network_error: 'A network error occurred. Please try again later.'},
}

const CodeBlueEventViewController: FC<any> = (props) => {
  const {BaseCrudViewService, modalTitle, itemData, setItemData, loading, entityId, reloadView, isShowView, handleCallbackFunc} =
    useCrudViewService(CodeBlueEventApi, initialState, props)
  useEffect(() => {
  ...
  return (
    <DrawerView loading={loading} reloadView={reloadView} isShowView={isShowView} modalTitle={modalTitle}
      itemData={itemData} component={CodeBlueEventView} handleCallbackFunc={handleCallbackFunc} />
  )
}
export default React.memo(CodeBlueEventViewController)
