...
const OrganogramListing: FC<any> = (props) => {
  const {
    loading,
    listData,
    selectedRowKeys,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ViewAction
          entityId={record.id}
          actionItem={OrganogramAction.COMMON_ACTION.VIEW}
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => (
        <ListItemAction
          entityId={record.id}
          actionList={OrganogramAction.LIST_ITEM_ACTION}
          handleCallbackFunc={handleCallbackFunc}
        />
      ),
    },
...
export default React.memo(OrganogramListing);
