...
const EmployeeListFilter: FC<any> = (props) => {
  const {Search} = Input
  const {Option} = Select
  ...
  return (
    <div className='p-6'>
      <Row gutter={[16, 16]}>
        ...
        <Col md={6} xs={12}>
          <CreateAction
            actionItem={EmployeeAction.COMMON_ACTION.CREATE}
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col md={2} xs={12}>
          <Form.Item name='designation_id' label='Designation'>
            <DesignationSelect
              designationId={filters.designation_id || undefined}
              placeholder='All'
              allowClear={true}
              onSelect={(value) => handleOnChanged('filter_designation_id', value)}
              onChange={(value) => {
                if (!value) handleOnChanged('filter_designation_id', '')
              }}
              onLoad={() => {}}
            />
          </Form.Item>
        ...
        <Col md={4} xs={12}>
          <div className='d-flex justify-content-end'>
            ...
            <BulkAction
              bulkAction={EmployeeAction.BULK_ACTION}
              handleCallbackFunc={handleCallbackFunc}
            />
          </div>
        </Col>
...
export default React.memo(EmployeeListFilter)
