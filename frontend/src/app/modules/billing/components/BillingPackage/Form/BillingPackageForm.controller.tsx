...
import BillingPackageAddOrEditForm from './BillingPackageForm.form'
import {useCrudFormService} from 'src/app/hooks/crud/useCrudFormService'

const initialState = {
  modalTitle: 'Create Billing Package',
  itemData: {},
  fields: {
    code: null,
    name: null,
...
const BillingPackageFormController: FC<any> = (props) => {
  const {
    BaseCrudFormService,
    entityId,
    modalTitle,
    ...
    handleSubmitFailed,
    handleCallbackFunc,
  } = useCrudFormService(BillingPackageApi, initialState, props)

  useEffect(() => {
    if (entityId && isShowForm) {
      setIsNewRecord(false)
      setModalTitle('Edit Billing Package')
      resetForm()
      loadData()
    } else {
      resetForm()
  ...
  return (
    <div className='form-page-container'>
      <DrawerForm
        ...
        component={BillingPackageAddOrEditForm}
...
export default React.memo(BillingPackageFormController)
