...
import ThanaWiseDisbursementListFilter from './ThanaWiseDisbursementList.filter';
import ThanaWiseDisbursementListing from './ThanaWiseDisbursementList.listing';
import ThanaWiseDisbursementListPagination from './ThanaWiseDisbursementList.pagination';
...
const ThanaWiseDisbursementListController: FC = () => {
  ...
  const { formRef, initialValues, handleChange, resetForm } = useForm(
    initialState.fields
  );
  ...
  const initExportData = async () => {
    ...
    await exportThanaWiseDisbursementList();
  };

  const loadData = (): void => {
    setLoading(true);
    ReportInvApi.getThanaWiseDisbursementReport(payload.current)
      .then((res) => {
  ...
  const exportThanaWiseDisbursementList = (): Promise<any> => {
    return new Promise<any>((resolve, reject) => {
      setExportLoading(true);
      ReportInvApi.getThanaWiseDisbursementReportExport(payload.current)
        .then((res) => {
          if (res.status == 200) {
            download(new Blob([res.data]), 'ThanaWiseDisbursementReport.xlsx', {
  ...
  return (
    <div className="card">
      ...
      >
        <ThanaWiseDisbursementListFilter
          formRef={formRef}
          initialValues={initialValues}
        ...
        <ListingComponent
          ...
          handleSubmit={handleSubmit}
        />
        <ThanaWiseDisbursementListPagination
          pagination={pagination}
          totalCount={totalCount}
...
export default ThanaWiseDisbursementListController;
