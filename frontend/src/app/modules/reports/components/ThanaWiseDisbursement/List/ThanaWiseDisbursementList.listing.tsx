...
const ThanaWiseDisbursementListing: FC<any> = (props) => {
  const { pagination, loading, listData, branchInfo } = props;
      ...
      <Card className="mt-2">
        <Spin spinning={loading}>
          <div className="listing-page-content listing-page-content-collectionTargetReport">
            <ReportHeader title={t('DMP Unit Wise Disbursement Report')} />
            <div className="text-center fw-bold">
              {t('DMP Unit')}: {branchInfo?.name}
            </div>
            ...
            >
              <thead>
                <tr>
                  ...
                  <th>{t('Last Disbursement Date')}</th>
                </tr>
              </thead>
                    ...
                    return (
                      <React.Fragment key={index}>
                        <tr key={`${index}`} style={{ fontSize: 15 }}>
                          ...
                          <td width={'20%'}>{itemData.total_requested_qty}</td>
                          <td width={'20%'}>{itemData.total_received_qty}</td>
                          <td width={'20%'}>{itemData.last_received_date}</td>
                        </tr>
                      </React.Fragment>
                    );
                  })}
...
export default React.memo(ThanaWiseDisbursementListing);
