...
import RequesterWiseDisbursementListFilter from './RequesterWiseDisbursementList.filter'
import RequesterWiseDisbursementListing from './RequesterWiseDisbursementList.listing'
import RequesterWiseDisbursementListPagination from './RequesterWiseDisbursementList.pagination'
...
const RequesterWiseDisbursementListController: FC = () => {
  ...
  const queryState = {
    ...
    filters: {
      user_id: queryParams?.user_id || initialState.filters.user_id,
    },
    pagination: {
  ...
  const [ListingComponent, setListingComponent] = useState<any>(RequesterWiseDisbursementListing)
  ...
  const initExportData = async () => {
    ...
    await exportRequesterWiseDisbursementList()
  }

  const loadData = (): void => {
    setLoading(true)
    ReportInvApi.getRequesterWiseDisbursementReport(payload.current)
      .then((res) => {
        setUserInfo(res.data.requesterInfo)
  ...
  const exportRequesterWiseDisbursementList = (): Promise<any> => {
    return new Promise<any>((resolve, reject) => {
      setExportLoading(true)
      ReportInvApi.getRequesterWiseDisbursementReportExport(payload.current)
        .then((res) => {
          if (res.status == 200) {
            download(new Blob([res.data]), 'RequesterWiseDisbursementReport.xlsx', {
  ...
  const handleUrl = (): void => {
    ...
    if (filters.user_id) {
      urlObject.user_id = filters.user_id
    }
  ...
  const processFilters = (): string => {
    ...
    if (filters.user_id) {
      filterString += " AND user_id='" + filters.user_id + "'"
    }
    return filterString
  }
  ...
  const processQueryParams = () => {
    ...
    if (filters.user_id) {
      filterString['user_id'] = filters.user_id
    }
  ...
  const handleReset = () => {
    ...
    setFilters({
      user_id: initialState.filters.user_id,
    })
    setPagination({
  ...
  return (
    <div className='card'>
      ...
      >
        <RequesterWiseDisbursementListFilter
          formRef={formRef}
          initialValues={initialValues}
        ...
        <ListingComponent
          ...
          handleSubmit={handleSubmit}
        />
        <RequesterWiseDisbursementListPagination
          pagination={pagination}
          totalCount={totalCount}
...
export default RequesterWiseDisbursementListController
