...
import ItemWiseDisbursementListFilter from './ItemWiseDisbursementList.filter'
import ItemWiseDisbursementListing from './ItemWiseDisbursementList.listing'
import ItemWiseDisbursementListPagination from './ItemWiseDisbursementList.pagination'
...
const ItemWiseDisbursementListController: FC = () => {
  ...
  const queryState = {
    ...
    filters: {
      // item_id: queryParams?.item_id || initialState.filters.item_id,
      item_id: queryParams?.item_id || initialState.filters.item_id,
    },
    pagination: {
  ...
  const [ListingComponent, setListingComponent] = useState<any>(ItemWiseDisbursementListing)
  ...
  const initExportData = async () => {
    ...
    await exportItemWiseDisbursementList()
  }

  const loadData = (): void => {
    setLoading(true)
    ReportInvApi.getItemWiseDisbursementReport(payload.current)
      .then((res) => {
  ...
  const exportItemWiseDisbursementList = (): Promise<any> => {
    return new Promise<any>((resolve, reject) => {
      setExportLoading(true)
      ReportInvApi.getItemWiseDisbursementReportExport(payload.current)
        .then((res) => {
          if (res.status == 200) {
            download(new Blob([res.data]), 'ItemWiseDisbursementReport.xlsx', {
  ...
  const handleUrl = (): void => {
    ...
    if (filters.item_id) {
      urlObject.item_id = filters.item_id
    }
  ...
  const processFilters = (): string => {
    ...
    if (filters.item_id) {
      filterString += ' AND item_id = (' + filters.item_id + ')'
    }
    return filterString
  }
  ...
  const processQueryParams = () => {
    ...
    if (filters.item_id) {
      filterString['item_id'] = filters.item_id
    }
  ...
  const handleReset = () => {
    ...
    setFilters({
      item_id: initialState.filters.item_id,
    })
    setPagination({
  ...
  return (
    <div className='card'>
      ...
      >
        <ItemWiseDisbursementListFilter
          formRef={formRef}
          initialValues={initialValues}
        ...
        <ListingComponent
          ...
          handleSubmit={handleSubmit}
        />
        <ItemWiseDisbursementListPagination
          pagination={pagination}
          totalCount={totalCount}
...
export default ItemWiseDisbursementListController
