...
import ItemRequisitionStatusListFilter from './ItemRequistionStatusList.filter'
import ItemRequisitionStatusListing from './ItemRequistionStatusList.listing'
import ItemRequisitionStatusListPagination from './ItemRequistionStatusList.pagination'
...
const ItemRequisitionStatusListController: FC = () => {
  ...
  const queryState = {
    ...
    filters: {
      ...
      // risk_status: queryParams?.risk_status || initialState.filters.risk_status,
  ...
  const [ListingComponent, setListingComponent] = useState<any>(ItemRequisitionStatusListing)
  ...
  const initExportData = async () => {
    ...
    await exportItemRequisitionStatusList()
  }

  const loadData = (): void => {
    setLoading(true)
    ReportInvApi.getItemRequisitionStatusReport(payload.current)
      .then((res) => {
  ...
  const exportItemRequisitionStatusList = (): Promise<any> => {
    return new Promise<any>((resolve, reject) => {
      setExportLoading(true)
      ReportInvApi.getItemRequisitionStatusReportExport(payload.current)
        .then((res) => {
          if (res.status == 200) {
            download(new Blob([res.data]), 'ItemRequisitionStatusReport.xlsx', {
  ...
  const handleUrl = (): void => {
    ...
    //   urlObject.risk_status = filters.risk_status;
    // }
  ...
  const processFilters = (): string => {
    ...
    // }
    // if (filters.risk_status) {
    //   filterString += " AND risk_status='" + filters.risk_status + "'";
    // }
    return filterString
  }
  ...
  const processQueryParams = () => {
    ...
    // }
    // if (filters.risk_status) {
    //   filterString['risk_status'] = filters.risk_status;
    // }
  ...
  return (
    <div className='card'>
      ...
      >
        <ItemRequisitionStatusListFilter
          formRef={formRef}
          initialValues={initialValues}
        ...
        <ItemRequisitionStatusListing
          ...
          handleSubmit={handleSubmit}
        />
        <ItemRequisitionStatusListPagination
          pagination={pagination}
          totalCount={totalCount}
...
export default ItemRequisitionStatusListController
