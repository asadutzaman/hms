...
const ResourceListFilter: FC<any> = (props) => {
  const { Search } = Input;
  const { Option } = Select;
  ...
  return (
    <div className="p-6">
      <Row gutter={[16, 16]}>
        ...
        <Col md={6} xs={12}>
          <CreateAction
            actionItem={ResourceAction.COMMON_ACTION.CREATE}
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col md={3} xs={12}>
          <Form.Item name="permission_type" label={t('Permission Type')}>
            <Select
              showSearch
              popupMatchSelectWidth={200}
              defaultValue={filters.permission_type}
              optionFilterProp="children"
              onChange={(value) =>
                handleOnChanged('filter_permission_type', value)
              }
              filterOption={(input, option: any) =>
                option?.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
              }
        ...
        <Col md={3} xs={12}>
          <div className="d-flex justify-content-end">
            ...
            <BulkAction
              bulkAction={ResourceAction.BULK_ACTION}
              handleCallbackFunc={handleCallbackFunc}
            />
          </div>
        </Col>
...
export default React.memo(ResourceListFilter);
