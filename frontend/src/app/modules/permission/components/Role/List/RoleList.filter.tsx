...
const RoleListFilter: FC<any> = (props) => {
  const { Search } = Input;
  const { Option } = Select;
        ...
        <Col md={6} xs={12}>
          ...
          >
            <h3 className="card-title align-items-start flex-column">
              <span className="card-label fw-bold fs-3 mb-1">{t('Roles')}</span>
            </h3>
          </div>
        </Col>
        <Col md={6} xs={12}>
          <CreateAction
            actionItem={RoleAction.COMMON_ACTION.CREATE}
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col md={4} xs={12}>
          <div className="d-flex justify-content-end">
            ...
            <BulkAction
              bulkAction={RoleAction.BULK_ACTION}
              handleCallbackFunc={handleCallbackFunc}
            />
          </div>
        </Col>
