...
const StockAdjustmentListFilter: FC<any> = (props) => {
  const { Search } = Input;
  const { Option } = Select;
  ...
  return (
    <div className="p-6">
      <Row gutter={[16, 16]}>
        ...
        <Col md={6} xs={12}>
          <CreateAction
            actionItem={StockAdjustmentAction.COMMON_ACTION.CREATE}
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col md={4} xs={12}>
          <Form.Item name="process_status" label={t('Process Status')}>
            <Select
              showSearch
              popupMatchSelectWidth={true}
              defaultValue={filters.process_status}
              optionFilterProp="children"
              onChange={(value) => handleOnChanged('process_status', value)}
...
export default React.memo(StockAdjustmentListFilter);
