...
import StockAdjustmentItemAddMoreItem from './StockAdjustmentItem.addMore.item';
...
const StockAdjustmentItemAddMore: FC<IProps> = (props) => {
    ...
    <table className="table table-bordered">
      <thead>
        <tr>
          ...
          <th style={{ width: '25%' }}>{t('Shelve')}</th>
          <th style={{ width: '15%' }}>{t('Quantity')}</th>
          <th style={{ width: '20%' }}>
            {t('Remarks')} <span className="text-danger required"></span>
          </th>
      ...
      <tbody>
        {addMoreItemList.length > 0 &&
          addMoreItemList.map((item: any, index: any) => (
            <StockAdjustmentItemAddMoreItem
...
export default React.memo(StockAdjustmentItemAddMore);
