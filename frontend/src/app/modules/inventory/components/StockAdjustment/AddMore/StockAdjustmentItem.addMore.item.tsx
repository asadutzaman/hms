...
const StockAdjustmentItemAddMoreItem: FC<any> = (props) => {
  const { Option } = Select;
  const {
    addMoreItemIndex,
    addMoreItem,
  ...
  return (
    <tr>
      ...
      <td>
        <Form.Item label={''} name="shelve_id" rules={rules.required}>
          <ShelveSelect
            shelveId={addMoreItem.shelve_id}
            placeholder={t('Select Shelve')}
            style={{ width: '100%' }}
...
export default React.memo(StockAdjustmentItemAddMoreItem);
