...
import WorkflowSingleActionModal from 'src/app/components/Workflow/Actions/WorkflowSingleAction.modal'
import {useForm} from 'src/app/hooks/useForm'
import ReviseRequisitionActionForm from './ReviseRequisitionAction.form'
import {RequisitionApprovalApi, WorkflowTransitionApi} from 'src/app/api'
...
const ReviseRequisitionActionController: FC<any> = (props) => {
  const {
    entityId,
    workflowLoading,
    workflowData,
  ...
  const loadData = (): Promise<any> => {
    return new Promise((resolve, reject) => {
      setLoading(true)
      RequisitionApprovalApi.getById(entityId)
        .then((res: any) => {
          setItemList(res.data?.requisitionItemsListData)
          setLoading(false)
          resolve(res)
        })
        .catch((err: any) => {
          if (err?.status === 409) {
            ...
            Message.error(typeof err.data === 'string' ? err.data : JSON.stringify(err.data))
          } else {
  ...
  return (
    <Fragment>
      <WorkflowSingleActionModal
        ...
        component={ReviseRequisitionActionForm}
        // itemData={itemData}
        itemList={itemList}
      ...
      >
        {workflowActionInfo.action_type == 'CUSTOM'
...
export default React.memo(ReviseRequisitionActionController)
