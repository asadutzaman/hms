...
import PurchaseOrderApprovalListFilter from './PurchaseOrderApprovalList.filter'
import PurchaseOrderApprovalListing from './PurchaseOrderApprovalList.listing'
import PurchaseOrderApprovalListPagination from './PurchaseOrderApprovalList.pagination'
...
const initialState = {
  ...
  message: {
    ...
    delete_success: 'Delete Purchase Order successfully.',
    ...
    delete_bulk_confirm: 'Are you sure you wish to delete selected Purchase Order?',
  },
}

const PurchaseOrderApprovalListController: FC<any> = (props) => {
  ...
  const {
    ...
    reloadView,
    reloadForm,
  } = useCrudListService(PurchaseOrderApprovalApi, queryState, initialState, props)
  ...
  const {activeStepData, getMyWorkflowActiveStepFilters, activeStepActionList, workflowNextStepApproverList} =
    useWorkflowActiveStep(workflowData, activeStep)

  useEffect(() => {
    if (workflowData && activeStep === '') {
      const firstActiveStep = workflowData.workflow_steps?.find((step) => step.myStep)?.step_code || ''
  ...
  return (
    <div className='card'>
      <Form form={formRef} name='exampleListingFilterForm' initialValues={initialValues}>
        <PurchaseOrderApprovalListFilter
          filters={filters}
        ...
        <PurchaseOrderApprovalListing
          ...
          handleCallbackFunc={handleCallbackFunc}
        />
        <PurchaseOrderApprovalListPagination
          pagination={pagination}
          totalCount={totalCount}
          handleOnChanged={handleOnChanged}
        />
      </Form>
      <PurchaseOrderApprovalViewController
        entityId={entityId}
...
export default PurchaseOrderApprovalListController
