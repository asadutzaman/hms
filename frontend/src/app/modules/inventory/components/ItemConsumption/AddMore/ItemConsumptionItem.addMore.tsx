...
import ItemConsumptionItemAddMoreItem from './ItemConsumptionItem.addMore.item'
...
const ItemConsumptionItemAddMore: FC<IProps> = (props) => {
  ...
  const handleAddMoreItemEdit = (fieldName: string, fieldValue: any, fieldIndex: any) => {
    setAddMoreItemList((addMoreItemList: any) => {
      if (fieldName === 'quantity') {
        ...
        if (fieldValue > addMoreItemList[fieldIndex]['balance_quantity']) {
          Message.error('Quantity cannot be greater than balance quantity')
          return [...addMoreItemList]
        }
    ...
    <table className='table table-bordered'>
      <thead>
        <tr>
          ...
          <th style={{width: '10%'}}>{t('Item Type')}</th>
          <th style={{width: '15%'}}>{t('Balance Quantity')}</th>
          <th style={{width: '15%'}}>{t('Consume Quantity')}</th>
      ...
      <tbody>
        {addMoreItemList.length > 0 &&
          addMoreItemList.map((item: any, index: any) => (
            <ItemConsumptionItemAddMoreItem
...
export default React.memo(ItemConsumptionItemAddMore)
