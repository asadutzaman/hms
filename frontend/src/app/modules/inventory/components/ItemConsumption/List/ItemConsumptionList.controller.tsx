...
import ItemConsumptionListFilter from './ItemConsumptionList.filter'
import ItemConsumptionListing from './ItemConsumptionList.listing'
import ItemConsumptionListPagination from './ItemConsumptionList.pagination'
...
const initialState = {
  ...
  message: {
    ...
    delete_success: 'Delete itemConsumption successfully.',
    delete_confirm_title: 'Delete ItemConsumption',
    delete_confirm: 'Are you sure you want to delete this itemConsumption?',
    delete_bulk_select: 'Please select item(s)',
    delete_bulk_confirm: 'Are you sure you wish to delete selected itemConsumption?',
  },
}

const ItemConsumptionListController: FC<any> = (props) => {
  ...
  const {
    ...
    reloadView,
    reloadForm,
  } = useCrudListService(ItemConsumptionApi, queryState, initialState, props)

  useEffect(() => {
    if (queryParams.newItemConsumption) {
      setIsShowForm(true)
    }
  }, [queryParams.newItemConsumption])
  ...
  return (
    <div className='card'>
      <Form form={formRef} name='exampleListingFilterForm' initialValues={initialValues}>
        <ItemConsumptionListFilter
          filters={filters}
          handleOnChanged={handleOnChanged}
          handleCallbackFunc={handleCallbackFunc}
        />
        <ItemConsumptionListing
          loading={loading}
          listData={listData}
          ...
          handleCallbackFunc={handleCallbackFunc}
        />
        <ItemConsumptionListPagination
          pagination={pagination}
          totalCount={totalCount}
          handleOnChanged={handleOnChanged}
        />
      </Form>
      <ItemConsumptionViewController
        entityId={entityId}
        reloadView={reloadView}
        isShowView={isShowView}
        handleCallbackFunc={handleCallbackFunc}
      />
      <ItemConsumptionFormController
        entityId={entityId}
...
export default ItemConsumptionListController
