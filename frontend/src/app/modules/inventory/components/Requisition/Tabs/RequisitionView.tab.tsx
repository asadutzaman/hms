...
import RequisitionItemViewTab from './RequisitionItemView.tab';
  ...
  return (
    <>
      <div className="container-fluid">
        ...
        <div className="d-flex justify-content-between align-items-start mb-10 ps-2">
          <div>
            ...
            <div className="fs-5 text-gray-700">
              <div className="fw-bolder">
                {t('Deputy Commissioner (Logistics)')}
              </div>
          ...
          >
            <div className="d-flex justify-content-between mb-1">
              <span className="fw-bolder text-gray-800 me-2">
                {t('Requisition No')}:
              </span>
              <span className="text-gray-700">
                {itemData.requisition_number}
              </span>
            </div>
            <div className="d-flex justify-content-between mb-1">
              <span className="fw-bolder text-gray-800 me-2">{t('Date')}:</span>
              <span className="text-gray-700">
                {DateTimeUtils.formatDate(itemData.created_at)}
              </span>
            </div>
              ...
              <span className="text-gray-700">
                {['APPROVED', 'DISBURSED'].includes(itemData.process_status) ? (
                  <span className="text-danger fw-bolder">
                    ...
                    {itemData.disbursement_start_time} -{' '}
                    {itemData.disbursement_end_time}
                  </span>
                ) : (
                  '-'
                )}
          ...
          <table className="table table-row-bordered table-row-gray-300 align-middle gs-0 gy-3">
            <thead>
              <tr className="fw-bolder bg-light">
                ...
                <th className="min-w-100px text-end pe-4">
                  {t('Requested / Revised Quantity')}
                </th>
                <th className="min-w-100px text-end pe-4">
                  {t('Receipts last month')}
                </th>
            ...
            <tbody>
              {itemData.requisitionItemsListData !== undefined &&
                itemData.requisitionItemsListData.map(
                  (item: any, index: any) => (
                    <tr key={`local-${index}`}>
        ...
        <div className="d-flex justify-content-between mt-15 px-10">
          ...
          <div className="text-start">
            <h5 className="fw-bolder text-gray-800 mb-4">
              {t('Approved By')}:
            </h5>
          ...
          <div className="text-start">
            ...
            <div className="d-flex flex-column">
              ...
              <div className="text-gray-600 fw-bold fs-6">
                {itemData.created_by_designation}
              </div>
      ...
      <table className='table table-row-dashed table-row-gray-300 align-middle gs-0 gy-1'>
        <tr>
          <td width={'20%'}>{t('Disbursement Date & Time')}</td>
          <td width={'5%'}>:</td>
          <td width={'75%'}>
            {['APPROVED', 'DISBURSED'].includes(itemData.process_status) ? (
              <span className='text-danger fw-bolder'>
        ...
        <tr>
          <td width={'20%'}>{t('Requisition Subject')}</td>
          <td width={'5%'}>:</td>
          <td width={'75%'}>{`[${itemData.requisition_number}] - ${itemData.subject}`}</td>
        </tr>
        <tr>
      ...
      <RequisitionItemViewTab itemData={itemData} {...restProps} />
    </div> */}
    </>
  );
};
