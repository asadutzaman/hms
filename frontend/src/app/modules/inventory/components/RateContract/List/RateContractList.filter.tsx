...
const RateContractListFilter: FC<any> = (props) => {
  const {Search} = Input
        ...
        <Col md={6} xs={12}>
          <div className='card card-header p-0 pb-3' style={{minHeight: '0px'}}>
            <h3 className='card-title align-items-start flex-column'>
              <span className='card-label fw-bold fs-3 mb-1'>{t('Rate Contract List')}</span>
            </h3>
          </div>
        </Col>
        <Col md={6} xs={12}>
          <CreateAction actionItem={RateContractAction.COMMON_ACTION.CREATE} handleCallbackFunc={handleCallbackFunc} />
        </Col>
      </Row>
      ...
      <Row>
        <Col md={4} xs={12}>
          <Form.Item name='search'>
            <Search placeholder={t('Search')} onSearch={(value) => handleOnChanged('search', value)} />
          </Form.Item>
        </Col>
        <Col md={8} xs={12}>
...
export default React.memo(RateContractListFilter)
