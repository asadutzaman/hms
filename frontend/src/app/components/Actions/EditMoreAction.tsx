...
const EditMoreAction: FC<IProps> = props => {
    const { entityIndex, btnText, children, component: Component, actionItem, manageUrl = true, handleCallbackFunc, ...restProps } = props;
    ...
    const showFormModal = (): void => {
        if (manageUrl && handleCallbackFunc) {
            handleCallbackFunc(null, 'edit', entityIndex);
        }
        else {
            setIsShowForm(true);
            handleReloadForm();
        ...
        return (
            <>
                {children ?
                    ...
                    :
                    (<Button type="primary" className="btn btn-edit btn-sm btn-light-primary me-3" onClick={() => showFormModal()}> {btnText || actionItem.title}</Button>)}
                {!manageUrl && <Component entityIndex={entityIndex} reloadForm={reloadForm} isShowForm={isShowForm} handleCallbackFunc={handleCallback}  {...restProps} />}
            </>
        );
    }
